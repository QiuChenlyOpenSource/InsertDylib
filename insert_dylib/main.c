...
#include "DeleteSegment.h"

#define IS_64_BIT(x) ((x) == MH_MAGIC_64 || (x) == MH_CIGAM_64)
#define IS_LITTLE_ENDIAN(x) ((x) == FAT_CIGAM_64 || (x) == FAT_CIGAM || (x) == MH_CIGAM_64 || (x) == MH_CIGAM)
...
static struct option long_options[] = {
        ...
        {"not-all-yes", no_argument, &yes_flag, false},
        {"delete_dylib", no_argument, &delete_flag, true},
        {NULL, 0, NULL, 0}
};
        ...
        switch (cmd) {
            case LC_CODE_SIGNATURE:
                if (i == ncmds - 1) {
                    ...
                    if (codesig_flag == 0 && !ask("发现代码加载签名: LC_CODE_SIGNATURE load command. 要自动删掉吗?")) {
                        return true;
                    }
...
int main(int argc, const char *argv[]) {
    ...
    if (!inplace_flag) {
        ...
        if (argc == 4) {
            ...
            if (!delete_flag)
                ...
                asprintf(&new_binary_path, "%s_removed", binary_path);
            binary_path_was_malloced = true;
        }

        if (!overwrite_flag && stat(new_binary_path, &s) == 0) {
            if (!ask("%s 目标文件已经存在. 要重写他吗?", new_binary_path)) {
                exit(1);
            }
        }
    ...
    if (delete_flag) {
        //原作者Tyilo纯c写的这玩意真的一团糟 改都懒得改了 纯浪费时间 不如用OC重写一个。
        bool r = delete_dylib(binary_path, dylib_path);
        if (r) {
            printf("删除%s %s的LC_Command成功。", dylib_path, binary_path);
        } else {
            printf("删除%s %s的LC_Command失败。", dylib_path, binary_path);
        }
        return 0;
    }
    ...
    switch (magic) {
        ...
        case FAT_CIGAM: {
            ...
            printf("这是一个FAT格式二进制,携带 %d 个架构的代码(Intel/Apple Silicon ARM64).\n", nfat_arch);
